...
| packages                | Directory | Nautobot apps (e.g., `nautobot-schedule`, test helpers).   |
...
```bash
uvx towncrier create --config pyproject.toml --dir docs/schedule 124.feature
uvx towncrier build --config pyproject.toml --dir docs/schedule --version 0.0.0
```
