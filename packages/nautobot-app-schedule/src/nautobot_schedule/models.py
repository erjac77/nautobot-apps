...
from nautobot_schedule.constants import FIRST_WEEK_DAY
from nautobot_schedule.validators import JSONSchemaValidator
...
class ScheduleType(PrimaryModel, models.Model):
...
@extras_features("custom_links", "custom_validators", "export_templates", "graphql", "webhooks")
class Schedule(PrimaryModel, models.Model):
    schedule_type = models.ForeignKey(
        ScheduleType, on_delete=models.CASCADE, related_name="schedule"
    )
...
class RecurrenceRule(PrimaryModel, models.Model):
    ...
    byday = models.JSONField(
        ...
        validators=[JSONSchemaValidator(schema=_byparams["byday"]["schema"])],
    )
    bymonthday = models.JSONField(
        ...
        validators=[JSONSchemaValidator(schema=_byparams["bymonthday"]["schema"])],
    )
    byyearday = models.JSONField(
        ...
        validators=[JSONSchemaValidator(schema=_byparams["byyearday"]["schema"])],
    )
    byweekno = models.JSONField(
        ...
        validators=[JSONSchemaValidator(schema=_byparams["byweekno"]["schema"])],
    )
    bymonth = models.JSONField(
        blank=True,
        default=list,
        help_text="A COMMA-separated list of months of the year. Valid values are 1 to 12.",
        validators=[JSONSchemaValidator(schema=_byparams["bymonth"]["schema"])],
    )
    bysetpos = models.JSONField(
        ...
        validators=[JSONSchemaValidator(schema=_byparams["bysetpos"]["schema"])],
    )
...
class Event(PrimaryModel, models.Model):
    ...
    exdates = models.JSONField(
        ...
        validators=[JSONSchemaValidator(schema=date_schema)],
    )
    ...
    def clean(self) -> None:
        ...
        if self.dtend < self.dtstart:
            raise ValidationError({"dtend": "The end date cannot be before the start date."})
